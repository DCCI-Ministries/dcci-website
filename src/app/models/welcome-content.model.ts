...
/** Max archived live pages kept for rollback (adminSettings/welcomeVersions/versions). */
export const MAX_WELCOME_VERSIONS = 10;

/** Archived published snapshot for rollback (adminSettings/welcomeVersions/versions). */
export interface WelcomePageVersionSummary {
  versionId: string;
  archivedAt: unknown;
  archivedBy?: string;
  /** System label when archived (e.g. "Before publish"). */
  label?: string;
  /** Admin-chosen title for this backup (optional). */
  displayTitle?: string;
  /** Admin-chosen note (optional; not shown on the public site). */
  displayDescription?: string;
}

/** Title shown in version history — custom title, else archived date, else system label. */
export function versionDisplayTitle(version: WelcomePageVersionSummary): string {
  const custom = (version.displayTitle || '').trim();
  if (custom) {
    return custom;
  }
...
/** Subtitle under the title — custom description, else system archive label. */
export function versionDisplaySubtitle(version: WelcomePageVersionSummary): string | null {
  const desc = (version.displayDescription || '').trim();
  if (desc) {
    return desc;
  }
  const auto = (version.label || '').trim();
  return auto || null;
}
